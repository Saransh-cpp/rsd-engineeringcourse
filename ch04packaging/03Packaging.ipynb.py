...
# These wheel files are installable via `pip`` and are generated by build backends. Wheels can be different for different platforms
# and different Python versions. `pip` only resorts to installing a library through its source distribution (often referred as `SDist`)
...
#  - [`setuptools`](https://brew.sh/): allows building pure Python and Python + C/C++ projects
#  - [`hatch`](https://hatch.pypa.io): allows building (and recommended for) pure Python projects
#  - [`flit`](https://chocolatey.org/) allows building pure Python projects with minimal extra configurations
#  - [`poetry`](https://chocolatey.org/) allows building pure Python projects (a full blown dependency and environment management system)
#  - [`scikit-build-core`](https://chocolatey.org/) allows building pure Python and Python + C/C++ projects (under active development)
#  - [`meson`](https://chocolatey.org/) allows building pure Python and Python + C/C++ projects (has a custom DSL)
#  - [`maturin`]: allows building Rust binary extensions
#
...
# %% [markdown]
# When installing the package now, pip will also install the dependencies automatically. The `-e` flag tells pip to make the installation "editable."
# Editable installs allow pip to automatically update the installed package with new changes everytime the interpreter is restarted.
